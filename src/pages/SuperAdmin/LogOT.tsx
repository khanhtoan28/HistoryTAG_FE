...
export default function SuperAdminLogOT() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setOffdayHours(summary.offdayHours || 0);
      setRequests((list.data || []) as OTRequest[]);
      setTotalItems(list.pagination?.totalItems || 0);
      setTotalPages(Math.max(1, list.pagination?.totalPages || 1));
    } catch (error) {
  ...
  useEffect(() => {
    loadData();
  }, [selectedMonth, selectedDept, search, currentPage]);
  ...
  const detailStatusLabel = detailRequest
    ? (STATUS_CONFIG[detailRequest.status]?.label ?? detailRequest.status)
    : "";

  const submittedOnPage = useMemo(
    () => requests.filter((r) => r.status === "submitted").map((r) => r.id),
    [requests]
  );
          ...
          <div className="flex flex-col items-center justify-between gap-4 border-t border-gray-200 px-4 py-3 dark:border-gray-800 sm:flex-row">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-lg border border-gray-300 bg-white text-gray-600 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
              >
                &lt;
              </button>
