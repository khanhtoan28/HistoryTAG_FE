...
const BusinessCalendar: React.FC = () => {
  ...
  useEffect(() => {
    const loadTeamMembers = async () => {
      ...
      try {
        // Load users from SALES team (Phòng Kinh Doanh)
  ...
  useEffect(() => {
    // Initialize with some events for Business team
    setEvents([
      {
        id: "1",
        title: "Họp phòng kinh doanh",
        start: new Date().toISOString().split("T")[0],
        extendedProps: { calendar: "Danger", team: "SALES" },
      },
      {
        id: "2",
        title: "Báo cáo doanh số",
        start: new Date(Date.now() + 86400000).toISOString().split("T")[0],
        extendedProps: { calendar: "Success", team: "SALES" },
      },
    ]);
  }, []);
  ...
  const resetModalFields = () => {
    ...
    setEventTime("");
    setEventLevel("");
    setEventType("team"); // Luôn mặc định là team
    setSelectedMemberId("");
  ...
  return (
    <>
      <PageMeta
        title="Lịch Phòng Kinh Doanh | TailAdmin"
        description="Lịch làm việc của phòng kinh doanh"
      />
