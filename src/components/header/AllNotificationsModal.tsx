...
import { getAllNotifications, getNotificationsPage, deleteNotification, markAsRead } from "../../api/notification.api";
...
export default function AllNotificationsModal({ isOpen, onClose }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getNotificationsPage(0, size);
  ...
  const handleOpenDetail = async (n: any) => {
    try {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-3xl rounded-2xl bg-white p-4 shadow-lg dark:bg-gray-900">
        ...
        <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
          ...
          <ul className="flex flex-col gap-2">
            {filtered.map((n) => (
              <li key={n.id} className="flex items-start gap-3 p-3 rounded border">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-gray-500 truncate max-w-[600px]">{n.message}</div>
                    </div>
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {!isAllLoaded && (
              <button onClick={() => loadAll()} className="px-3 py-1 text-sm border rounded">Load All</button>
            )}
